...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def save_cookbook_state(request: Request):
        ...
        try:
            ...
            # vault pattern says N<total AND no DOWNLOAD_OK/DOWNLOAD_FAILED/
            ...
            for _it in incoming_tasks:
                ...
                if ("DOWNLOAD_OK" in _out) or ("DOWNLOAD_FAILED" in _out) or ("/snapshots/" in _out):
                    continue
                _vaults = _re_dl.findall(r"model-(\d+)-of-(\d+)\.safetensors", _out)
                if _vaults:
                    _n, _tot = _vaults[-1]
                    if int(_n) < int(_tot):
                        logger.info(f"cookbook state POST: rejecting stale done for {_it.get('sessionId')} "
                                    f"(last vault {_n}/{_tot}, no DOWNLOAD_OK)")
                        _it["status"] = "running"
                else:
    ...
    def _cookbook_tasks_status_sync():
        ...
        def _download_cache_incomplete(repo_id: str, remote_host: str = "", ssh_port: str = "", cache_root: str = "") -> bool:
            ...
            py = (
                ...
                "d=os.path.join(base,'models--'+repo.replace('/','--'));"
                "blobs=os.path.join(d,'blobs');"
                "sys.exit(0 if os.path.isdir(blobs) and any(x.endswith('.incomplete') for x in os.listdir(blobs)) else 1)"
            )
            if remote_host:
                cmd = ["python3", "-c", py, repo_id]
            else:
