...
export class ChatPanelProvider implements vscode.WebviewViewProvider {
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (msg: { type: string; text?: string; model?: string }) => {
      switch (msg.type) {
        case 'send':
          if (msg.text) { await this._handleSend(msg.text); }
          break;
        case 'newSession':
  ...
  private async _handleNewSession(model?: string): Promise<void> {
    try {
  ...
  private async _loadModels(): Promise<void> {
    try {
      ...
      const models = result.items.flatMap(ep =>
        (ep.models || []).map(m => ({
          id: m,
          name: m.split('/').pop() || m,
