"""Tests for Obsidian vault file watcher and link extraction."""
...
from src.obsidian_watcher import (
    _parse_frontmatter,
    _parse_yaml_tags,
    _extract_links,
    _resolve_obsidian_link,
    _sync_file,
...
def test_sync_file_with_links(tmp_path):
    from core.database import SessionLocal, Obsidian, engine, Base
    ...
    db = SessionLocal()
    try:
        a = db.query(Obsidian).filter_by(rel_path="A.md").first()
        assert a is not None
        assert a.title == "Note A"
        assert json.loads(a.tags) == ["alpha"]
        # B may not resolve yet if file doesn't exist
        b = db.query(Obsidian).filter_by(rel_path="B.md").first()
